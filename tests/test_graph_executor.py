...
def test_dispatch_action_run_tool(graph_executor):
    """Test _dispatch_action invokes registry for run_tool action."""
    node = {
        "action": "run_tool",
        "params": {
            "tool": "my_tool",
    ...
    # Mock registry invoke
    graph_executor.registry.invoke = MagicMock(return_value={"status": "success"})

    graph_executor._dispatch_action(node, context)

    graph_executor.registry.invoke.assert_called_once_with("my_tool", arg1="val1")

def test_dispatch_action_shizuku_injection(graph_executor):
    """Test _dispatch_action injects use_root=True when shizuku_active is in context."""
    node = {
        "action": "run_tool",
        "params": {
            "tool": "my_tool",
    ...
    graph_executor.registry.invoke.assert_called_once_with("my_tool", arg1="val1", use_root=True)

def test_dispatch_action_missing_params(graph_executor):
    """Test _dispatch_action raises ValueError when params are missing."""
    node = {
        "action": "run_tool"
        # Missing "params" key
    }
    ...
    # Expect ValueError for better error handling
    with pytest.raises(ValueError, match="Missing parameters for tool execution"):
        graph_executor._dispatch_action(node, context)

def test_dispatch_action_missing_tool(graph_executor):
    """Test _dispatch_action raises ValueError when tool name is missing in params."""
    node = {
        "action": "run_tool",
        "params": {}
        # Missing "tool" key inside params
    }
    context = {}

    with pytest.raises(ValueError, match="Tool name missing in run_tool action"):
        graph_executor._dispatch_action(node, context)

def test_dispatch_action_non_tool_action(graph_executor):
    """Test _dispatch_action returns success for non-tool actions."""
    node = {
        "action": "logic_gate"
    }
    context = {}

    result = graph_executor._dispatch_action(node, context)
